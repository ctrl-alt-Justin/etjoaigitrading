...
export default async function ShopCatalogPage() {
  const [{ items: forSale, categories }, storefrontSettings] = await Promise.all([
    getShopCatalogData(),
    getStorefrontSettings(),
  ]);

  const banner = storefrontSettings.catalogBanner;
  const heroPhoto = banner.bgPhotoUrl?.trim() || forSale.find((item) => item.photos?.[0]?.url)?.photos?.[0]?.url;
  ...
  return (
    <div className="min-h-screen bg-[#FCFDF8] text-[#17364b] antialiased">
      ...
      <main className="mx-auto max-w-[1480px] pb-24">
        {/* Promotional Hero Banner Matching Mockups & Dashboard Config */}
          ...
          <div className="relative flex h-full flex-col justify-between px-6 py-8 sm:px-12">
            ...
            <div className="text-left">
              <div className="font-hegarty text-xs font-black uppercase tracking-wider text-white sm:text-sm">
                {banner.productName || "LANDSKRONA"}
              </div>
              <div className="text-[11px] font-medium text-stone-200">
                {banner.productDesc || "2-seat sofa, dark blue velvet."}
              </div>
              <div className="mt-0.5 text-xs font-bold text-white">
                {banner.productPrice || "4799"}
              </div>
            </div>
            ...
            <div className="text-left">
              <h1 className="font-hegarty text-2xl font-black uppercase tracking-tight text-white sm:text-4xl">
                {banner.promoTitle || "SUPER SUMMER SALE"}
              </h1>
              <p className="mt-0.5 text-xs font-bold uppercase tracking-wider text-[#16c4df] sm:text-sm">
                {banner.promoSubtitle || "UP TO 50% OFF ON SELECTED PRE-OWNED ITEMS"}
              </p>
            </div>
          </div>
        </section>
